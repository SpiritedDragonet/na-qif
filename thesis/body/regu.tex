...
第三章将给出本文端到端模型的统一表述：包括时间 bin 离散与输入—输出形式的等价关系，链路中各物理模块的量子线路化表示（幺正门与量子信道/Kraus 算符），以及成功事件集合与条件化输出态的严格定义，为后续算法与仿真结果建立清晰的理论接口。\cite{Pichler2016TimeDelaysMPS,vanleent2022entangling}。%
