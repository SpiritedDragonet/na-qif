# -*- coding: utf-8 -*-
"""
QFC_NOISE_SCAN 任务：按 QFC 背景噪声谱密度扫描运行 SIM 核心。
"""
...
def run_qfc_noise_scan_task(
    ...
    if not isinstance(payload, dict) or "qfc_noise_sd_cps_per_mhz" not in payload:
        raise ValueError("SCHEMA_ERROR: QFC_NOISE_SCAN task 缺少 payload.qfc_noise_sd_cps_per_mhz")
    qfc_noise_sd_cps_per_mhz = float(payload["qfc_noise_sd_cps_per_mhz"])
    if qfc_noise_sd_cps_per_mhz < 0.0:
        raise ValueError("QFC_NOISE_SCAN 的 qfc_noise_sd_cps_per_mhz 必须 >= 0")
    ...
    if enum_main is None:
        raise RuntimeError("QFC_NOISE_SCAN 需要 compute_metrics=True 且返回有效枚举结果")
    ...
    metrics = {
        "run_index": run_index,
        "qfc_noise_levels": [entry],
    }
    write_click_records(raw_dir, {f"{float(qfc_noise_sd_cps_per_mhz):.9f}": click_records})
    write_declared_density_matrix(
        raw_dir,
