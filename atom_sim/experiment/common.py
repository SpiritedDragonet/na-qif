...
# 以当前 n_bins=100, dt=1ns 的输出统计为基线，
# 将 delay=0 的峰位放在“提前(99%保留)”与“延后(80%保留)”裕度中点附近。
DEFAULT_EMISSION_T0_NS = 8.0
DEFAULT_EMISSION_OMEGA_PEAK_RAD_S = float(2.0 * np.pi * 20e6 * 0.65)
DEFAULT_EMISSION_G_RAD_S = float(2.0 * np.pi * 20e6 * 0.09)
DEFAULT_EMISSION_KAPPA_EX_HZ = 20e6
DEFAULT_EMISSION_KAPPA_IN_HZ = 1e6
