...
# **把“几何/收集/偏振定义”压缩成一个 2×2 复矩阵 (\Alpha^{(\mathrm{emit})})：** 它把原子自然基（(\sigma^\pm)）映射到你数值里用的光路偏振基（H/V）。我们约定 [ \Alpha^{(\mathrm{emit})}
...
# **time-bin 离散后的局域发射门：** [ U^{(\mathrm{emit})}_{j,n}
...
# 把 QFC 最小可信地写成“频域 beam splitter”幺正：对每偏振 (p), [ U^{(\mathrm{QFC})}_{j,n}
...
# 对到达同一 bin (n) 的两臂 telecom 模式做 50/50 beamsplitter 干涉：对每偏振 (p), [ \begin{pmatrix} d^{p}*{1,n}\ d^{p}*{2,n} \end{pmatrix}
